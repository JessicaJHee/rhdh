...
| **`RHDH_ROOT`** | No | Absolute path to the RHDH repo root if omitted from context. |
...
```bash
cd /abs/rhdh && HUSKY=0 git fetch <RHDH_CORE_REMOTE> release-<V> && HUSKY=0 git checkout release-<V> && HUSKY=0 git pull <RHDH_CORE_REMOTE> release-<V>
```
...
- **Batch** related shell commands with **`&&`** and **`cd /abs/...`**; cwd may not persist between tool calls. Use **`network` / `git_write` / `all`** as needed (**`all`** for **`rm`/`cp`** into Yarn patch temps or stubborn sandboxes).
